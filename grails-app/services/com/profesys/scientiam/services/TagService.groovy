...
//		   workEffortInstance.workEffortName = 'Generación por Historia de usuario'
		   ...
		   workEffortInstance.id  =  TransactionService.serviceID('STORY',session.user.toString() )
		   workEffortInstance.userCreated     = User.read ( session.user.id )
...
//		   workEffortInstance.actualSetupMillis= 0
		   ...
		   workEffortInstance.scopeEnum = Enumeration.read('PUBLIC')
...
//		   storyInstance.userCreated     =  User.read ( session.user.id )
   ...
   @Transactional
   def save(Story storyInstance,User userCreation,User userUpdate) {
	   def result = [:]
	   def fail = { Map m ->
		   result.error = [ code: m.code, args: ["Story"] ]
		   return result
